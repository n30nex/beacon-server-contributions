...
func TestRuntimeConfigUpdate(t *testing.T) {
	newRouter := func() http.Handler {
		return New(nil, nil, nil, 5, 10, config.CORSConfig{AllowedOrigins: []string{"https://before.test"}}, config.ServerConfig{}, config.ResolvedRateLimitConfig{}, config.AuthConfig{APIKey: "test-key"}, nil)
	}
	handler := newRouter()
