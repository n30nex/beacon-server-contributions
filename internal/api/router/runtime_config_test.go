...
func TestRuntimeConfigUpdate(t *testing.T) {
	newRouter := func() http.Handler {
		return New(nil, nil, nil, 5, 1000, config.CORSConfig{AllowedOrigins: []string{"https://before.test"}}, config.ServerConfig{}, config.AuthConfig{APIKey: "test-key"}, config.ResolvedRateLimitConfig{}, nil)
	}
	handler := newRouter()
