...
func checkWebSocketLimit(t *testing.T, cfg config.ServerConfig, secondIP string, wantShed bool) {
	t.Helper()
	server := httptest.NewServer(New(hub.New(), nil, nil, 1, 10, config.CORSConfig{}, cfg, config.ResolvedRateLimitConfig{Enabled: true, RequestsPerMinute: 1, Burst: 1}, config.AuthConfig{}))
	defer server.Close()
